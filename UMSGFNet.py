...
class HGNNEncoder(nn.Module):
    ...
    def __init__(self, atom_fdim, bond_fdim, hidden_size, depth, device):
        ...
        self.W_b = KAN([self.hidden_size, self.hidden_size])
...
class FHGNN(nn.Module):
    def __init__(self,
        ...
        # )
        self.mlp_fp = nn.Sequential(
            KAN([fp_fdim, 1024, 2048]),
            nn.ReLU(),
            nn.Dropout(0.1),
            KAN([2048, 1024, 1024]),
            nn.ReLU(),
            nn.Dropout(0.1),
            KAN([1024, hidden_size])
        )
    ...
    def forward(self, batch):
        ...
        # 使用自注意力机制
        ligand_x = ligand_x.unsqueeze(0)  # 添加batch维度
        fp_x = fp_x.unsqueeze(0)  # 添加batch维度
        attn_output, _ = self.attention(ligand_x, fp_x, fp_x)
        ligand_x = attn_output.squeeze(0)  # 去除batch维度
