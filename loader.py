...
def _load_tox21_dataset(input_path):
    """
    :param input_path: tox21 CSV 文件路径
    ...
    if 'smiles' in input_df.columns:
        ...
        # 如果没有显式的 "smiles" 列，则取最后一列作为 SMILES
        smiles_list = input_df.iloc[:, -1]

    # 2. 清理 SMILES 列：去掉 NaN、空字符串
    smiles_list = smiles_list.fillna('').astype(str)
    smiles_list = [s.strip() for s in smiles_list]

    # 3. 生成 rdkit 分子对象（非法 SMILES 会变成 None）
    rdkit_mol_objs_list = []
    valid_smiles_list = []
    for smi in smiles_list:
        ...
        if mol is not None and smi != '':
            ...
            print(f"[Warning] 无法解析 SMILES: {smi}, 已跳过")
    ...
    # 同步 labels（只保留合法的分子）
    labels = labels.loc[[i for i, smi in enumerate(smiles_list) if AllChem.MolFromSmiles(smi) is not None]]

    assert len(valid_smiles_list) == len(rdkit_mol_objs_list)
    assert len(valid_smiles_list) == len(labels)

    return valid_smiles_list, rdkit_mol_objs_list, labels.values
